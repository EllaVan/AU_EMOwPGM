...
from InferPGM.base import factor_product
    ...
    def _variable_elimination(
        ...
        if joint:
            if isinstance(self.model, BayesianNetwork):
                # return factor_product(*final_distribution).normalize(inplace=False) #原句为此，有修改
                return_values = factor_product(*final_distribution)
                return_variable = DiscreteFactor(variables, [self.cardinality[variables[0]]], return_values)
                return return_variable.normalize(inplace=False)
            else:
