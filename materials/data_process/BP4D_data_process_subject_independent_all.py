...
def get_unseen_fake_rules(num_unseen, input_rules):
    ...
    EMO2AU_cpt = np.concatenate([EMO2AU_cpt, fake_EMO2AU_cpt])
    EMO = EMO + unseen_nodes
    ...
    for k in range(num_EMO):
        AU_pos_nonzero = np.nonzero(EMO2AU_cpt[k])[0]  #EMO=k的AU-EMO关联（非0值）的位置
        AU_pos_certain = np.where(EMO2AU_cpt[k]==1)[0]  #EMO=k的AU-EMO关联（1值）的位置
        for j in range(len(AU_pos_nonzero)):
            if EMO2AU_cpt[k][AU_pos_nonzero[j]] == 1.0:  #当AUj是确定发生的时候，AUi同时发生的概率就是AUi自己本身的值
                for i in range(len(AU_pos_nonzero)):
                    if i != j:
                        AU_cpt_tmp[AU_pos_nonzero[i]][AU_pos_nonzero[j]] += EMO2AU_cpt[k][AU_pos_nonzero[i]]
...
def get_priori():
    ...
    for au_ij in range(AU_cpt.shape[0]):
        AU_ij_cnt[:, au_ij] = AU_cpt[:, au_ij] * AU_cnt[au_ij]
    input_rules = EMO2AU_cpt, AU_cpt, prob_AU, ori_size, num_all_img, AU_ij_cnt, AU_cnt, EMO, AU
    # input_rules = get_unseen_fake_rules(len(unseen_nodes), input_rules)
    return input_rules

input_rules = get_priori()
BP4D_pkl['priori'] = input_rules
EMO2AU_cpt, AU_cpt, prob_AU, ori_size, num_all_img, AU_ij_cnt, AU_cnt, EMO, AU = input_rules
EMO = seen_nodes + unseen_nodes
...
with open('/media/data1/wf/AU_EMOwPGM/codes/dataset/BP4D/BP4D_all_subject_independent_v2.pkl', 'wb') as fo:
