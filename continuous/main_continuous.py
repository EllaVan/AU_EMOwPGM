...
def main(conf):
    ...
    for dataset_i, dataset_name in enumerate(conf.dataset_order):
        ...
        if dataset_i > 0:
            ...
            output_rules, train_records, model = learn_rules(conf, train_rules_input, priori_rules, latest_rules, AU_p_d, summary_writer, -1, 0.01)
        ...
        if dataset_i >= 1:
            for cur_i, (AU_p_d, val_rules_input, loc1, loc2) in enumerate(for_all_test):
