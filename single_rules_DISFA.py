...
def learn_rules(conf, device, input_info, input_rules, AU_p_d, summary_writer, *args):
    ...
    loc1 = [0, 1, 2, 3, 4, 6, 9, 10, 11, 12, 15, 16]
    loc2 = [5, 7, 8, 13, 14]
    EMO2AU = EMO2AU_cpt
    ...
    for idx in range(labelsAU.shape[0]):
        ...
        if len(occ_au) != 0:
            num_all_img += 1
            prob_all_au = RadiateAUs(conf, emo_label, AU_cpt, occ_au, loc2, EMO2AU, thresh=0.6)
    ...
    EMO2AU_cpt1 = update.EMO2AU_cpt.data.detach().cpu().numpy()
    EMO2AU_cpt2 = update.static_EMO2AU_cpt.data.detach().cpu().numpy()
    EMO2AU_cpt[:, loc1] = EMO2AU_cpt1
    EMO2AU_cpt[:, loc2] = EMO2AU_cpt2
    prob_AU = np.zeros((len(AU),))
    prob_AU1 = update.prob_AU.data.detach().cpu().numpy()
    prob_AU2 = update.static_prob_AU.data.detach().cpu().numpy()
...
def test_rules(conf, update, device, input_info, input_rules, AU_p_d, summary_writer, *args):
...
def main_rules(conf, device, cur_path, info_source, AU_p_d):
    ...
    rules_summary_path = '/media/data1/wf/AU_EMOwPGM/codes/results/tmp_DISFA_v3'
...
def main(conf):
    ...
    pre_path = '/media/data1/wf/AU_EMOwPGM/codes/results0911_theTrust/DISFA/Test/subject_independent/bs_64_seed_0_lrEMO_0.0003_lrAU_0.0001_lr_relation_0.001'
    conf.outdir = pre_path
    # file_list = walkFile(pre_path)
    file_list = ['epoch4_model_fold0.pth']
