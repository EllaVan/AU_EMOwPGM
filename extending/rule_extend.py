...
class UpdateGraph_proj(nn.Module):
    def __init__(self, conf, input_rules, loc1, loc2, proj_W=None, temp=1):
        super(UpdateGraph_proj, self).__init__()
        self.conf = conf
        self.loc1 = loc1
        self.loc2 = loc2
        self.temp = temp
...
def learn_rules_KL(conf, device, input_info, input_rules, seen_trained_rules, AU_p_d, summary_writer, *args):
    device = conf.device
    loc1 = conf.loc1
    loc2 = conf.loc2
    labelsAU, labelsEMO = input_info
    ...
    seen_trained_model = UpdateGraph_proj(conf, seen_trained_rules, loc1, loc2, temp=2).to(device)
    ...
    num_EMO = EMO2AU_cpt.shape[0]
    confu_m = torch.zeros((num_EMO, num_EMO))
    update = UpdateGraph_proj(conf, input_rules, loc1, loc2, temp=2).to(device)
