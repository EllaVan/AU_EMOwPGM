...
def main(conf):
    ...
    for dataset_i, dataset_name in enumerate(conf.dataset_order):
        ...
        repeat_size = int((unseen_train_inputEMO.shape[0]/num_unseen*num_seen)// len(samplesEMO1) / 2)
        ...
        repeat_size = int((part2_unseen_trainEMO.shape[0]/num_unseen*num_seen)// len(samplesEMO1))
        samplesAU = samplesAU1.repeat(repeat_size, 1)
        samplesEMO = torch.concat(samplesEMO1 * repeat_size)
