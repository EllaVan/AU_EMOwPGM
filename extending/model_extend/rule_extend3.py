...
class UpdateGraph(nn.Module):
    def __init__(self, conf, input_rules, proj_W=None, temp=1):
        ...
        self.register_buffer('neg_static_EMO2AU_cpt', torch.from_numpy(1 - EMO2AU_cpt[:, self.loc2]))
...
def learn_rules(conf, input_info, input_rules, seen_trained_rules, AU_p_d, summary_writer, *args):
    ...
    seen_trained_model = UpdateGraph(conf, seen_trained_rules, loc1, loc2, temp=2).to(device)
    ...
    update = UpdateGraph(conf, input_rules, loc1, loc2, temp=2).to(device)
