        ...
        if len(occ_au) >= 3:
            ...
            for i in range(prob_all_au.shape[0]-2):
                if i in pos:
                    ...
                    # if if_bonus is True and i in occ_au:
                    #         AU2EMO_bonus = np.ones(len(EMO), )
                    #         latent_pos = weight[i].argsort()
                    #         for j, bonus_emo in enumerate(latent_pos):··
                    #             if j == 0:
                    #                 AU2EMO_bonus[bonus_emo] = 1
                    #             else:
                    #                 if weight[i][latent_pos[j]] == weight[i][latent_pos[j-1]]:
                    #                     AU2EMO_bonus[bonus_emo] = AU2EMO_bonus[latent_pos[j-1]]
                    #                 else:
                    #                     AU2EMO_bonus[bonus_emo] = j+1
                    #         EMO_bonus = EMO_bonus * AU2EMO_bonus
                else:
                    prob_all_au[i] = 1 / (1-prob_AU[i])
                    weight[i, :] = 1 - weight[i, :]
            
            # EMO_bonus = torch.Tensor(EMO_bonus).reshape(1, -1).to(device)
            ...
            summary_writer.add_scalar('err', np.array(err_record).mean(), num_all_img-ori_size)
            summary_writer.add_scalar('err_single', utils.loss_to_float(err), num_all_img-ori_size)
            summary_writer.add_scalar('acc', np.array(acc_record).mean(), num_all_img-ori_size)
