...
def learn_rules(conf, device, input_info, input_rules, summary_writer, AU_p_d, *args):
    ...
    if args:
        change_weight1 = 1560 / (ori_size + train_size)
        change_weight2 = 1
...
def main_rules(conf, device, cur_path, info_source, AU_p_d):
    ...
    input_rules, output_rules, train_records = learn_rules(conf, device, train_rules_input, input_rules, summary_writer)#, change_w)
    train_rules_loss, train_rules_acc, train_confu_m = train_records
    val_records = test_rules(conf, device, val_rules_input, output_rules, summary_writer)
    # val_records2 = test_rules(conf, device, val_rules_input, input_rules, summary_writer)
...
def main(conf):
    ...
    pre_path = '/media/data1/wf/AU_EMOwPGM/codes/results0911/RAF-DB/Test/subject_independent/bs_64_seed_0_lrEMO_0.0003_lrAU_0.0001_lr_relation_0.001'
    conf.outdir = pre_path
    # file_list = walkFile(pre_path)
    file_list = ['epoch8_model_fold0.pth']
    ...
    for info_source in source_list:
        for f in file_list:
            ...
            train_rules_loss, train_rules_acc, val_rules_loss, val_rules_acc, val_confu_m = main_continuous(conf, device, f, info_source, AU_p_d)
